...
public class SeguimientoService {
    ...
    @Value("${ms-rutas.url:http://localhost:8084}")
    private String msRutasUrl;
    ...
    public SeguimientoDto obtenerSeguimiento(Integer solicitudId) {
        ...
        if (solicitud.getIdRuta() != null) {
            ...
            List<TramoResponse> tramos = obtenerTramosDeRuta(solicitud.getIdRuta());
            
            for (TramoResponse tramo : tramos) {
                if (tramo.getFechaHoraInicio() != null) {
                    historial.add(SeguimientoDto.EstadoHistorial.builder()
                            .estado("EN_TRANSITO")
                            .descripcion("Tramo iniciado - " + obtenerTipoTramo(tramo.getTipoTramoId()))
                            .fecha(tramo.getFechaHoraInicio())
                            .ubicacion("Ubicación " + tramo.getUbicacionOrigenId())
                            .build());
                }
                ...
                if (tramo.getFechaHoraFin() != null) {
                    historial.add(SeguimientoDto.EstadoHistorial.builder()
                            .estado("ARRIBO")
                            .descripcion("Llegada a destino del tramo")
                            .fecha(tramo.getFechaHoraFin())
                            .ubicacion("Ubicación " + tramo.getUbicacionDestinoId())
                            .build());
                }
            }
        }
    ...
    private List<TramoResponse> obtenerTramosDeRuta(Integer rutaId) {
        try {
            String url = msRutasUrl + "/api/tramos/ruta/" + rutaId;
            TramoResponse[] tramos = webClientBuilder.build()
                    .get()
                    .uri(url)
                    .retrieve()
                    .bodyToMono(TramoResponse[].class)
                    .block();

            return tramos != null ? List.of(tramos) : new ArrayList<>();
        } catch (Exception e) {
            log.warn("Error al obtener tramos de ruta {}: {}", rutaId, e.getMessage());
            return new ArrayList<>();
        }
    }

    private String obtenerTipoTramo(Integer tipoTramoId) {
        switch (tipoTramoId) {
            case 1: return "Origen a Depósito";
            case 2: return "Depósito a Depósito";
            case 3: return "Depósito a Destino";
            case 4: return "Origen a Destino";
            default: return "Tramo " + tipoTramoId;
        }
    }
    ...
    @lombok.Data
    private static class TramoResponse {
