...
CREATE TABLE Ubicacion (
    -- Ahora la columna se genera automáticamente usando IDENTITY
    ubicacion_id INTEGER GENERATED BY DEFAULT AS IDENTITY,
...
CREATE TABLE Deposito (
    deposito_id INTEGER GENERATED BY DEFAULT AS IDENTITY,
...
-- Compatibilidad para la secuencia de ubicacion_id (idempotente y seguro)
DO $$
DECLARE
    seq_reg TEXT;
    v_max INTEGER;
BEGIN
    -- Buscar la secuencia asociada a public.ubicacion.ubicacion_id si existe
    SELECT pg_get_serial_sequence('public.ubicacion','ubicacion_id') INTO seq_reg;

    -- Si no se encontró con pg_get_serial_sequence, buscar por dependencia (cubre identity y casos manuales)
    IF seq_reg IS NULL THEN
        SELECT format('public.%s', c.relname) INTO seq_reg
        FROM pg_class c
        JOIN pg_depend d ON d.objid = c.oid AND d.deptype = 'a'
        JOIN pg_class t ON d.refobjid = t.oid
        JOIN pg_attribute a ON a.attrelid = t.oid AND a.attnum = d.refobjsubid
        WHERE t.relname = 'ubicacion' AND a.attname = 'ubicacion_id'
        LIMIT 1;
    END IF;

    -- Si no existe secuencia, crear una nueva con nombre predecible
    IF seq_reg IS NULL THEN
        seq_reg := 'public.ubicacion_ubicacion_id_seq';
        EXECUTE 'CREATE SEQUENCE IF NOT EXISTS ' || seq_reg;
    END IF;

    -- Obtener el máximo id actual
    SELECT MAX(ubicacion_id) INTO v_max FROM public.ubicacion;

    IF v_max IS NULL OR v_max < 1 THEN
        -- Si no hay filas, inicializamos la secuencia para que nextval devuelva 1
        EXECUTE format('SELECT setval(%L, 1, false)', seq_reg);
        RAISE NOTICE 'Secuencia % inicializada para tabla vacía', seq_reg;
    ELSE
        -- Ajustamos la secuencia al MAX actual (is_called = true para que nextval devuelva max+1)
        EXECUTE format('SELECT setval(%L, %s, true)', seq_reg, v_max);
        RAISE NOTICE 'Secuencia % ajustada a %', seq_reg, v_max;
    END IF;
END$$;
...
-- Compatibilidad para la secuencia de deposito_id (idempotente y seguro)
DO $$
DECLARE
    seq_reg TEXT;
    v_max INTEGER;
BEGIN
    -- Buscar la secuencia asociada a public.deposito.deposito_id si existe
    SELECT pg_get_serial_sequence('public.deposito','deposito_id') INTO seq_reg;

    -- Si no se encontró con pg_get_serial_sequence, buscar por dependencia
    IF seq_reg IS NULL THEN
        SELECT format('public.%s', c.relname) INTO seq_reg
        FROM pg_class c
        JOIN pg_depend d ON d.objid = c.oid AND d.deptype = 'a'
        JOIN pg_class t ON d.refobjid = t.oid
        JOIN pg_attribute a ON a.attrelid = t.oid AND a.attnum = d.refobjsubid
        WHERE t.relname = 'deposito' AND a.attname = 'deposito_id'
        LIMIT 1;
    END IF;

    -- Si no existe secuencia, crear una nueva con nombre predecible
    IF seq_reg IS NULL THEN
        seq_reg := 'public.deposito_deposito_id_seq';
        EXECUTE 'CREATE SEQUENCE IF NOT EXISTS ' || seq_reg;
    END IF;

    -- Obtener el máximo id actual
    SELECT MAX(deposito_id) INTO v_max FROM public.deposito;
